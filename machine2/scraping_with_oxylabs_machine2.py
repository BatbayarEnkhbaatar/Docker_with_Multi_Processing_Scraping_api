...
logging.basicConfig(level=logging.DEBUG, filename="./machine2.log", format="%(asctime)s:%(levelno)s:%(message)s")
# Opening JSON file
ua = UserAgent()
...
while one_time_max < end:
    try:
        ...
        rdf.to_excel(f'RESULTS/result_machine2_{req_num}.xlsx', index=False)
        result_list.clear()
        ...
        num_of_fail +=1
        print("Failed attempt :", num_of_fail)
