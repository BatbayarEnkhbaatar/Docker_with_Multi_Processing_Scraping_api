...
logging.basicConfig(level=logging.DEBUG, filename="./machine3.log", format="%(asctime)s:%(levelno)s:%(message)s")
# Opening JSON file
ua = UserAgent()
...
while one_time_max < end:
    try:
        ...
        rdf.to_excel(f'RESULTS/result_machine3_{req_num}.xlsx', index=False)
        result_list.clear()
